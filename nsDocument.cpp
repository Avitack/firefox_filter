                              ...
                              bool aReset, nsIContentSink* aSink)
{
  ...
  nsCOMPtr<nsIURI> uri;
  aChannel->GetURI(getter_AddRefs(uri));
  //printf("\n XXX StartDocumentLoad, uri: %s XXX", uri->GetSpecOrDefault().get());
  if (mScriptLoader) {
    bool filterStatus = mScriptLoader->xssFilter->FetchRequestData(this);
    if (filterStatus) {
      //printf("\n XSSFilter: Status OK");
    } else {
      printf("\n XSSFilter: Status ERROR");
    }
  }
  //nsresult rv = InitXSSFilter(aChannel);
  //NS_ENSURE_SUCCESS(rv, rv);
...
// nsresult
// nsIDocument::InitXSSFilter(nsIChannel* aChannel){

//   nsCOMPtr<nsIHttpChannel> httpChannel;
//   nsresult rv = GetHttpChannelHelper(aChannel, getter_AddRefs(httpChannel));
//   if (NS_WARN_IF(NS_FAILED(rv))) {
//     printf("\n XSSFilter NOT initialized with document! XXXXXXXXXXXXXXXXXXXXXXXXXXXXxx");
//     return rv;
//   }

//   printf("\n XSSFilter initialized with document!");
//   xssFilter = new dom::XSSFilter(this);
//   return NS_OK;
// }
