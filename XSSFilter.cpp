...
XSSFilter::XSSFilter(nsIDocument *aDocument)
  : mDocument(aDocument)
{
  //printf("\n XSSFilter init");
  if (aDocument) {
...
bool
XSSFilter::FetchRequestData(nsIDocument* aDocument) {
  mDocument = aDocument;

  if (aDocument) {
    //printf("\n aDocument OK, fetch data next");
    GetPOSTData();
    GetGETData();
    TrimRequestData();
    return true;
...
XSSFilter::isInjected(nsCString reqParam, nsCString mScriptContent) {
  ...
  //printf("\n Param: %s \n compared with \n %s", reqParam.get(), mScriptContent.get());
...
XSSFilter::TrimRequestData() {
  ...
  nsString value, key;
  for (int i = 0; i < urlPostParams.Length(); i++) {
  ...
  for (int i = 0; i < urlGetParams.Length(); i++) {
...
bool
XSSFilter::StartFilterInternalScript(nsCString mScriptContent) {
...
XSSFilter::GetPOSTData() {
  ...
  if (mDocument) {
    ...
    //nsresult rvChannel = mDocument->GetHttpChannelHelper(mDocument->GetChannel(), getter_AddRefs(mHttpChannel));
    //if (NS_WARN_IF(NS_FAILED(rvChannel))) {
    //    return;
    //}
    nsCOMPtr<nsIChannel> mChannel = mDocument->GetChannel();
  if (mChannel) {
    ...
    if (mHttpChannel) {
      ...
      if (mUploadChannel) {
        ...
        nsresult rv = mBodyStream->Available(&mAvailableData);
...
bool
XSSFilter::GetGETData() {

  // Get URL search params (GET)
  nsAutoCString docURL;
  if(mDocument) {
    nsIURI* mURI = mDocument->GetDocumentURI();
    if (mURI) {
      mURI->GetSpec(docURL);

    nsAutoCString path;
    nsresult rv = mDocument->GetDocumentURI()->GetPathQueryRef(path);

    int32_t queryBegins = path.FindChar('?');
    if (queryBegins > 0) {
        urlGetParams.ParseInput(Substring(path, queryBegins + 1));
        isGetParamSet = true;
        //printf("\n GET params fetched");
    }
    }
