    ...
    mReporter(new ConsoleReportCollector())
{
  ...
  xssFilter = new XSSFilter(mDocument);
}
                                    ...
                                    nsIContent* aScriptContent)
{
  ...
  // -------------- XSS Filter ---------------
  if (!xssFilter->StartFilterExternalScript(scriptURI)) {
    return false;
  }
        ...
        if (aRequest->IsBytecode()) {
...

          {
            ...
            if (aRequest->mOffThreadToken) {
              ...
              // Get script data
              nsString mScriptContentNSString =
                  nsDependentString(srcBuf.get(), srcBuf.length());
              nsCString mScriptContent =
                  NS_ConvertUTF16toUTF8(mScriptContentNSString);
              
              // -------------- XSS Filter ---------------
              bool isSafe = false;
              if (xssFilter != nullptr) {
                isSafe = xssFilter->StartFilterInternalScript(mScriptContent);
              } else {
                // Don't break scripts on site
                isSafe = true;
              }
              //printf("\nInline script: %s", mScriptContent.get());
